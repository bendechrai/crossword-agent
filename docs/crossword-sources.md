...
- Licence/terms: tooling is MIT licensed, but the crossword content itself remains under the original publishers' copyright. Only two things are freely downloadable: `xd-puzzles.zip` (over 6,000 pre-1965 New York Times puzzles, i.e. old enough to be public domain, confirmed reachable) and `xd-clues.zip` (over 6,000,000 answer/clue usages grouped by publication-year, listed on the data page but too large to verify a full download within the session timeout). The bulk of the corpus (post-1965, still-copyrighted puzzles) lives in a private "gxd" repo that requires requesting access via the project's Discord - it is not openly redistributable
- Fetch: direct zip downloads, e.g. `https://xd.saul.pw/xd-metadata.zip` and `https://xd.saul.pw/xd-puzzles.zip` (both confirmed HTTP 200); full/private corpus requires the gated `gxd` repo
...
1. **The xd crossword corpus (Saul Pwanson)** is the broadest source by raw variety: 30+ American publications spanning 1990-2026, from the New York Times and LA Times down to indie constructors, all normalized into one consistent, git-diffable text format with the solution grid included. The major caveat is licensing: only a small, public-domain-era slice (about 6,000 pre-1965 NYT puzzles) and the aggregated clue-usage data are freely downloadable; the bulk of the modern, copyrighted corpus sits in a private repo gated behind a Discord access request.
...
- https://xd.saul.pw/data - fetched, confirmed public download links (xd-metadata.zip, xd-puzzles.zip "over 6000 pre-1965 New York Times crossword puzzles", xd-clues.zip "over 6,000,000 answer/clue usages")
