...
`decide` is a pure function of its context, which makes the whole escalation policy a table-driven unit test. It is consulted after **every** `getCandidates` return and once more at search termination for each still-empty slot. `domainSize` is the count of live candidates matching `currentPattern`, not the raw domain size, so triggers 1, 2 and 5 fire for a slot whose surviving candidates all conflict with the board. A re-ask counts as available only while `reasksPerSlot` remains **and** `patternFixedLetters > 0`: under `reask-first` an empty domain with no fixed letter therefore escalates at once, subject to the caps, rather than idling until trigger 5.
...
**2. Tier routing.** `tierRouter` maps `req.tier` to a model id from the profile (`tier1` default `deepseek-ai/DeepSeek-V4-Flash-0731` (T69), `tier2` default `deepseek-ai/DeepSeek-V4-Pro`) and picks the transport **by capability, not by model name**: it reads `supported_features` for that model from `models.json`, and if `structured_outputs` is present it sends `response_format: { type: 'json_schema', json_schema: { name: 'candidate_response', schema: <candidate-response.schema.json>, strict: true } }`; otherwise it puts the schema inline in the prompt with a one-shot example. The `{ name, schema, strict }` wrapper (rather than the schema document sent directly as `json_schema`) was confirmed against the live Nebius API in M2 (T49): Nebius's own request validator rejects the bare schema document with a "field required: name" error and accepts the wrapped form, which M2 also confirmed produces a real, correctly-parsed completion end to end (see docs/spikes/tier1-reliability.md section 2). Swapping either tier's model in a profile therefore cannot silently send a request form the model does not support.

Two other parameters are capability-gated the same way. For every tier-1 call when the model advertises `reasoning` (tier 2 keeps the seed-only rule until measured; see docs/spikes/tier1-reliability.md, 2026-09-04 follow-up), the router sends the provider's reasoning-off (or minimal-reasoning) parameter, because chain-of-thought on a "list ten six-letter answers" task buys nothing and is billed as completion tokens. M2 (T49) found this parameter empirically: `reasoning_effort`, whose value is per model - `"none"` by default, with an override table in `tierRouter.ts` for models known in advance to reject it (`openai/gpt-oss-120b`, a Harmony-format model whose reasoning levels are `low`/`medium`/`high` only, gets `"low"`) - and a client-side fallback (T68) that retries once with `"low"` when an HTTP 400 names `reasoning_effort` for a model not yet in that table. Nebius's own request validator names the full accepted literal set (`"none" | "minimal" | "low" | "medium" | "high" | "xhigh" | "max"`) on a 422 for an invalid value; `"none"` is the one M2 measured to actually drive the usage blob's `reasoningTokens` to 0 (the model otherwise spends its entire `sampling.maxTokens` completion budget on chain-of-thought before emitting any JSON - see docs/spikes/tier1-reliability.md section 1 for the before/after figures). When the model advertises `seed`, `--seed` is passed through; otherwise it is not.

**Constrained calls: reasoning and sampling (T71).** Two optional profile fields change what a constrained call is, and both are off by default, so a profile that sets neither behaves exactly as it did before them. `reasoning.constrainedEffort` (`none` | `low` | `medium` | `high`, default `none`) turns reasoning back **on** for the calls that render the `constrained` or `escalate` template - re-ask, repair, escalate - on a model that advertises `reasoning`: the router sends that value as `reasoning_effort` through the same per-model table above (`none` is the only value the table ever substitutes; `low`/`medium`/`high` are Harmony's own levels and pass through unchanged) and raises that call's `max_tokens` to `reasoning.constrainedMaxTokens` (default 2048), because a model that thinks needs room for the thinking and the JSON. This is where CrossWordBench expects the payoff (docs/crossword-algorithms.md: reasoning models improve as crossing letters are supplied, non-reasoning models do not), and it is deliberately not applied to the seed pass, where M2 measured chain-of-thought consuming the whole completion budget before any JSON appeared. `constrainedSamples` (1 to 5, default 1) asks the same constrained question that many times - each sample a request of its own, with its own `sampleIndex`, cache key, inference-log record and `slot:ask` event, issued one after another so the trace order is deterministic - and merges the accepted answers by vote: an answer's score is its best single-sample score plus 0.15 for each additional sample that proposed it, capped at 1.0, ordered by votes, then score, then the answer, and announced as one `slot:candidates`. That is the "sampling agreement" calibration of the algorithms doc, spent where a solver can afford it: on the minority of slots that reach a re-ask, rather than on every seed ask (that knob is `samples` with `calibration: 'votes'`). Both fields reach the cache key: `max_tokens` is a B23 key field already, and the effective effort is hashed with the B23 key when, and only when, one applies - so a key produced with reasoning off is byte-identical to the key the same request produced before T71.
...
5. **Re-ask (the sweep).** A domain that empties triggers a tier-1 re-ask (`purpose: 'reask'`, `promptKind: 'constrained'`) with the current pattern plus the rejected list, provided the pattern has at least one fixed letter, differs from the last pattern queried for that slot, and the slot is under `reasksPerSlot` (default 2). "Empty" is measured after the pattern filter - a domain whose every live candidate conflicts with the letters on the board is empty, since that is what the search branches on - and every crossing a single assignment empties is offered, not only the first. A pattern with no fixed letter has nothing a constrained re-ask could add, so such a slot goes straight to step 6. The result merges into the base domain and survives backtracking. `[slot:reask]`
6. **Escalate.** `policy/escalation.ts` exposes the pure `decide(ctx)` above, consulted after every `getCandidates` return and once at search termination for each still-empty slot. It owns all caps. At termination the still-empty slots are taken in descending fixed-letter order, so what is left of `maxTier2CallsPerPuzzle` is spent on the slots that can tell the strong model something: every constrained slot claims its call before any all-`?` slot is offered one, and an all-`?` escalation is skipped exactly when those better candidates have used the allowance up, never while calls are still going spare. An action `decide` chose that the search hooks cannot carry out is announced as `policy:refused` rather than disappearing into the `none` the search is handed. `[slot:escalate, policy:refused, budget:hit]`
...
The current `tier1` default is **`deepseek-ai/DeepSeek-V4-Flash-0731`** (T69), not the `nvidia/Nemotron-3_5-Lightning` shown in the illustrative code above: the seed-only recall screen (docs/benches/recall-screen.md) shortlisted it and the puzzle-level bench (docs/benches/model-comparison.md) confirmed it, measuring 0.80 letters accuracy on the american stratum against the prior default's 0.58, winning 24 of 24 paired repeats at about half the cost per puzzle. `ProfileSchema`'s `tier1` default and every built-in above carry the new value except `strong-only`, which already ran `deepseek-ai/DeepSeek-V4-Pro` for both tiers; the prior default stays selectable by writing `tier1` explicitly in a profile file.

**Two optional fields, and the profiles that use them (T71).** A `Profile` carries two fields the illustrative code above does not show, and they are the only two that may be absent: `constrainedSamples` (integer 1 to 5) and the `reasoning` group (`constrainedEffort`: `none` | `low` | `medium` | `high`; `constrainedMaxTokens`: integer, default 2048). Absent means the documented default - one sample, reasoning off - which is what every profile did before they existed, so every built-in above is unchanged and every run record already written stays valid against `schemas/run-record.schema.json`. What they do to a constrained call is described under "Candidate service" above; `src/profiles/schema.ts` exports `reasoningOf()` and `constrainedSamplesOf()`, which are the only places "absent" is turned into a value. Two built-ins set them. **`max-accuracy`** is the profile for a run where the answer matters more than the bill: `strong-only`'s models (`deepseek-ai/DeepSeek-V4-Pro` in both tiers) with `escalation.maxTier2CallsPerPuzzle` 200, `escalationsPerSlot` 2, `reasksPerSlot` 3, `search.maxBacktracks` 1000, `repair.maxCalls` 200, `budget` 2.0 USD and 10M tokens, `reasoning.constrainedEffort` `medium` and `constrainedSamples` 3 - caps lifted far enough that the run ends because the search is done rather than because a counter ran out, while a runaway still stops. **`strong-only-uncapped`** is its control: the same models, the same lifted caps and the same budget, with reasoning off and one sample, so a bench of the two separates what the caps bought from what the reasoning and the votes bought. Neither has been benched yet.
...
The same per-slot generation metrics (`truthInCandidates`, `truthRank`, `candidatesSeen`, `rejectCounts`, parse failures) are also produced without a solver at all by the seed-only candidate recall screen, `npx tsx scripts/eval-recall.ts` (aggregation in `eval/recall.ts`), which runs one tier-1 seed ask per slot per model through the real candidate service and cache and ranks models on truth-in-candidates share before any of them is worth a puzzle-level bench run; see docs/benches/README.md, "Seed-only candidate recall screen", for its command, metric definitions and decision rule.
...
`npm run fixtures:refresh` regenerates the committed cache and the snapshots together, so the two can never drift apart. A `promptVersion` bump invalidates every cached entry and is therefore a single-owner action: it lands with the regenerated cache in one commit, never as a code-only change that leaves the fixtures stale. The current `promptVersion` is **`"2"`** (T66): T65 shipped version 3 (version 2 with the per-answer letter-count self-check removed) as the default on the strength of an eight-puzzle analysis, but a follow-up paired measurement (72 runs, three repeats each) found version 2 beats version 3 on every accuracy metric, with confidence intervals excluding zero (american-stratum letters 0.5786 versus 0.5207 - see docs/benches/escalation-policy.md), so version 2 is the default again. Version 3 stays renderable so the comparison can be re-run - the built-in `baseline-pv3` is `baseline` with `promptVersion` `"3"` and nothing else changed - and `src/candidates/service.ts` selects the template by the resolved profile's `promptVersion`, which is the same value the cache key carries. `src/llm/prompts.ts` owns both version strings as `PROMPT_VERSION` (`"3"`) and `PAIRED_PROMPT_VERSION` (`"2"`, B49); `profiles/builtins.ts` picks one of the two for every built-in and `profiles/schema.ts`'s default does the same, so the profile field that reaches the cache key never spells a version number out itself.
...
| Tier-1 default (2026-09-05, T69) | `deepseek-ai/DeepSeek-V4-Flash-0731`, replacing `nvidia/Nemotron-3_5-Lightning` in `ProfileSchema`'s default and every built-in except `strong-only` | The puzzle-level bench (docs/benches/model-comparison.md), following the seed-only recall screen that shortlisted it (docs/benches/recall-screen.md), measured 0.80 letters accuracy on the american stratum against the prior default's 0.58, winning 24 of 24 paired repeats at about half the cost per puzzle; the old model stays selectable by writing `tier1` explicitly in a profile file. |
