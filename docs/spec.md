...
`npm run fixtures:refresh` regenerates the committed cache and the snapshots together, so the two can never drift apart. A `promptVersion` bump invalidates every cached entry and is therefore a single-owner action: it lands with the regenerated cache in one commit, never as a code-only change that leaves the fixtures stale. The current `promptVersion` is **`"2"`** (`PROMPT_VERSION` in `src/llm/prompts.ts`), bumped from `"1"` by T63 for the per-answer letter-count self-check and the reworded `clue_understood` scale.
