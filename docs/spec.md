...
`npm run fixtures:refresh` regenerates the committed cache and the snapshots together, so the two can never drift apart. A `promptVersion` bump invalidates every cached entry and is therefore a single-owner action: it lands with the regenerated cache in one commit, never as a code-only change that leaves the fixtures stale. The current `promptVersion` is **`"3"`** (T65): version 2 with the per-answer letter-count self-check removed, keeping the exact-length restatement and the reworded `clue_understood` scale that T63 introduced alongside it. A paired analysis of eight modern puzzles (docs/benches/escalation-policy.md, "Decomposition of the drop") tied about three quarters of a real slot-level regression to that one instruction: the model returned 35% fewer raw candidates and 41% fewer completion tokens per call, and short answers lost recall. Version 2 stays renderable so the difference can be measured rather than argued about - the built-in `baseline-pv2` is `baseline` with `promptVersion` `"2"` and nothing else changed, for exactly that paired run - and `src/candidates/service.ts` selects the template by the resolved profile's `promptVersion`, which is the same value the cache key carries. `PROMPT_VERSION` in `src/llm/prompts.ts` is the only place a version is written: the profile field that reaches the cache key (`profiles/builtins.ts`, and the `profiles/schema.ts` default) and the inference log's copy (`llm/client.ts`) all import it, so the prompt bytes and the key can never disagree.
