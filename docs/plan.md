...
### T62: Make the constrained re-ask fire before escalation; spend tier-2 on constrained slots
- Workstream: G (solver)
- Model: opus
- Depends on: T18, T37, T38, T44
- Owns: src/policy/escalation.ts, src/solver/hooks.ts, src/solver/search.ts, src/profiles/builtins.ts (tier-2 cap only), test/unit/policy/escalation.test.ts, test/unit/solver/hooks.test.ts, test/unit/solver/search.test.ts, test/unit/profiles/builtins.test.ts; pre-authorised: src/events/types.ts only to add an optional event for a refused policy action if none exists (`policy:refused` at level 2 with slotId, action, reason) plus its MIN_LEVEL entry, src/policy/types.ts only if EscalationContext needs a `patternFilteredDomainSize` field (prefer reusing domainSize with the new semantics if the doc comment allows), docs/spec.md only the Solver pipeline step 5/6 sentences and the Escalation triggers paragraph, docs/plan.md (this block and its index row).
- Reads (must not edit): src/solver/solve.ts, src/grid/domainStore.ts, src/candidates/service.ts, docs/crossword-algorithms.md (Constrained re-ask), docs/spec.md (Solver pipeline, Escalation).
- Spec sections: Solver pipeline; Escalation to the stronger model.
- Deliverable: An investigation of the canonical bench (36 runs) found the constrained re-ask fires on about 3% of slots although wipeouts run at 20 per puzzle, because: (1) at seed time an empty domain yields decide() -> reask under reask-first, the fixed-letter guard in hooks.ts reaskBlockedBy refuses it (no letters fixed yet), notExecuted collapses that to none, and solve.ts discards the verdict, so the slot idles until trigger 5 escalates it at termination with an all-? pattern (232 of 312 escalation prompts had zero fixed letters); (2) hooks.ts contextFor reports domains.sizeOf (unfiltered) while search.ts valuesFor is pattern-filtered, so a live-but-incompatible domain never triggers 1 or 2; (3) search.ts forward-check offers only the FIRST emptied crossing to onEmptyDomain (the `if (failed !== null) continue` short-circuit); (4) tryRefill reads only give-up and no event records a refused action. Implement: (a) escalation.ts desiredAction: a re-ask is only 'available' when ctx.patternFixedLetters > 0; with no fixed letters under reask-first an empty domain escalates immediately (spec step 2 already says a slot empty after validation goes to the escalation queue) subject to the caps; (b) hooks.ts contextFor: domainSize is the count of live candidates matching grid.patternFor(slotId) (patternMatches is already imported), document it in the context type's doc comment; (c) search.ts: offer every emptied crossing to onEmptyDomain (keep the first as the backtrack target); (d) emit a level-2 event when a policy action is refused by a guard (new event type if none fits) so refusals are visible in traces and run records; (e) at termination, order trigger-5 escalations by fixed-letter count descending so the tier-2 budget goes to constrained slots first, and skip a trigger-5 escalation whose pattern has zero fixed letters when the cap would be exhausted by better candidates (document the rule); (f) raise maxTier2CallsPerPuzzle from 15 to 25 in the baseline, eager-escalation and patient built-ins, with the profile tests updated and a one-line note in the builtins doc comment on why. Keep every cap (reasksPerSlot, escalationsPerSlot, maxTier2CallsPerPuzzle, budgets) enforced.
- Decisions baked in: no prompt or cache-key change; the trigger-4 proxy and clue_understood trigger are left as they are (T63 handles the confidence anchoring); tests are unit level with fakes, plus one offline solve on the synthetic 7x7 asserting at least one slot:reask event with a pattern carrying a fixed letter if the fixture produces a wipeout (if it cannot, say so and assert on the unit fixtures only).
- Acceptance: 1. escalation tests: {domainSize 0, patternFixedLetters 0, reask-first} -> escalate; {domainSize 0, patternFixedLetters 2, reasksUsed 0} -> reask; caps still downgrade. 2. hooks tests: an empty result at seed with an all-? pattern issues exactly one tier-2 request under reask-first and none with escalationsPerSlot 0; a live-but-pattern-incompatible domain issues one constrained re-ask with the current pattern and rejected list; the same-pattern guard still refuses; a refused action emits the refusal event. 3. search test: a forward check emptying two crossings calls onEmptyDomain twice. 4. termination ordering test: with a cap of 1 remaining and two empty slots, the one with fixed letters is escalated. 5. builtins tests reflect the cap of 25. 6. preflight passes; integration snapshots regenerated offline if cost or fill fields changed (accuracy must not regress on the synthetic fixtures; report before/after).
- Out of scope: prompts, cache, bench runs.
### T63: Seed prompt: length discipline and unanchored clue_understood (promptVersion 2)
- Workstream: E (prompting)
- Model: opus
- Depends on: T31, T49, T58
- Owns: src/llm/prompts.ts, test/unit/llm/prompts.test.ts, test/fixtures/prompts/**, test/fixtures/cache/** (regenerate), test/fixtures/runs/** (regenerate)
- Reads (must not edit): src/llm/parser.ts, src/validate/normalise.ts, src/candidates/service.ts, docs/spikes/tier1-reliability.md, docs/crossword-algorithms.md (Candidate generation)
- Spec sections: Candidate service (prompt shape); Strategy profiles (promptVersion)
- Deliverable: Two measured problems in the seed prompt. (1) Length: 85% of all candidate rejections in the canonical bench are wrong-length answers (the spike measured 13.5% top-candidate and 66.8% all-candidate length errors), so of 10 requested candidates only about 3 survive. Tighten the instruction: state the exact letter count once more immediately before the answer field, ask the model to write each answer's letter count next to it and to drop any that do not match before responding (a self-check that costs a few tokens), and show the one-shot example doing exactly that. Keep the JSON schema the parser expects (extra fields per candidate are ignored by the parser; confirm by reading parser.ts, and if it validates strictly, add the count field to the schema it validates against in your owned files only, else keep the count inside the answer's sibling field the schema already allows). (2) clue_understood: every parsed seed response in the bench reported 0.9 (5,258 of 5,279) because the few-shot examples hard-code 0.9/0.9/0.7, so trigger 3 (threshold 0.4) never fires. Rewrite the guidance to describe the scale in words (1.0 only when the clue is unambiguous and the top answer is certain; around 0.5 when the clue is understood but the answer is a guess; below 0.3 when the clue itself is opaque) and vary the examples so they are not all high. Bump promptVersion to "2" (you are its single owner); update the golden prompt fixtures; then refresh the committed synthetic cache and snapshots against Nebius and confirm strict offline replay still holds for both synthetic fixtures and that accuracy on them did not regress (report before and after, including how many candidates now survive validation per slot on the synthetic fixtures, from the run record's candidatesSeen).
...
- Decision baked in (2026-09-04, fixture clue fix): the synthetic 5x5's 5A clue "Synthetic silk-like fabric" became "Artificial silk made from cellulose" in every derived fixture (.json, .ipuz, .jpz, .xd, the .puz and its in-test generator, the events fixture and the prompt goldens), because the old clue was simply wrong - rayon is regenerated cellulose, not a synthetic, and NYLON fits it at least as well - so promptVersion 2 seeded NYLON, never listed RAYON, and the miss cascaded into 6 wipeouts and 3 unassigned slots; the fixture was at fault rather than the prompt, so the clue was fixed and no samples were re-rolled. The 5x5 now replays at letters=1.0 words=1.0 under strict offline (was 0.9545/0.8182 before promptVersion 2, 0.6818/0.5455 with the bad clue).
...
### T64: modern-12 bench set with fetch recipe; modern-source documentation
- Workstream: J
- Model: sonnet
- Depends on: T57, T60
- Owns: sets/modern-12.json, docs/benches/SETS.md, test/contract/sets.test.ts (additions), docs/crossword-sources.md (Crosshare entry and a new 'Modern puzzles for benches' subsection), docs/benches/README.md (one paragraph), README.md (one paragraph)
- Reads (must not edit): sets/mixed-12.json, src/sources/xd.ts, src/sources/guardian.ts, src/cli/fetch.ts, docs/build-notes/wave-2.md T27/T28/T29
- Spec sections: Strategy profiles (puzzle sets); Puzzle library and sources
- Deliverable, part 1 (step 3 of Ben's plan): make modern puzzles the standard bench data without distributing them. Create sets/modern-12.json in the same schema as sets/mixed-12.json: name "modern-12", exactly these entries: xd-lat2024-01-08, xd-lat2024-02-14, xd-lat2024-03-21, xd-lat2024-04-26, xd-lat2024-06-10, xd-lat2024-07-17, xd-lat2024-09-11, xd-lat2024-11-14 as stratum american (2024 Los Angeles Times weekday dailies from the xd corpus), and guardian-cryptic-30100, guardian-cryptic-30101, guardian-cryptic-30102, guardian-cryptic-30103 as stratum cryptic; a `note` field saying puzzles are fetched locally on demand and never committed, with a pointer to docs/benches/SETS.md. Extend test/contract/sets.test.ts with modern-12 cases (12 entries, 8/4 split, id/stratum keys only, unique ids) and a generic case over every sets/*.json file asserting no entry id begins with a real-publisher prefix that would indicate a committed puzzle (ids are fine; the rule is that sets never contain puzzle content: only id and stratum keys). Write docs/benches/SETS.md: the no-distribution policy in one paragraph; for each committed set (mixed-30 placeholders, mixed-12, modern-12) the exact reproducible fetch recipe: the xd corpus download URL (https://xd.saul.pw/xd-puzzles.zip, about 175 MB, saved to corpora/, gitignored), then one `docker exec crossword-solver xw fetch xd --path corpora/xd-puzzles.zip --from <date> --to <date> --limit 400` line per LA Times date (the xd source has no publisher filter, so the single-day window fetches every publisher for that day and only the lat id is used), and `docker exec crossword-solver xw fetch guardian --series cryptic --limit 4` with the note that the Guardian listing walks backward from the latest id so the four cryptic ids are reproducible only while they remain the latest four (state the ids and that a later re-fetch may need a larger --limit, never above 20); how to verify with `xw list --json`; and a reminder that runs, logs and cache stay local. Update docs/benches/README.md with one paragraph saying modern-12 is the standard set for tuning and that the escalation-policy result on mixed-12 predates the T62/T63 fixes and is kept as the historical baseline; do not touch its results sections. README.md: one paragraph under the puzzles section pointing to docs/benches/SETS.md.
- Deliverable, part 2 (step 4 of Ben's plan): document the modern-source findings. In docs/crossword-sources.md, replace the Crosshare entry's fetch guidance with the verified facts: crosshare.org's robots.txt (fetched 2026-09-04) disallows the whole site for any generic user agent and explicitly disallows /api/puz/, /api/pdf/ and /api/feed/ even for the named search bots; its terms grant personal transitory viewing only; the site is open source (github.com/crosshare-org/crosshare, AGPL-3.0) and the .puz export route exists but using it would defy the stated crawling policy; conclusion: not a source this project will fetch from; manual download by a human for personal use remains possible outside the tool. Add a subsection 'Modern puzzles for benches' listing the two sources the tool uses today: the xd corpus archive with date filters (modern dailies from 32 publishers; local personal research only), and the Guardian quick series via `xw fetch guardian --series quick` (definitional clues, lands with style quick, no adapter change needed), plus the free constructor .puz feeds (Brendan Emmett Quigley, Club72) as candidates that need their own terms check before an adapter is written. Keep every URL you cite to ones already verified in the doc's verification log or add a dated entry for the robots.txt fetch.
- Decisions baked in: no puzzle content in any committed file; no new adapter; Crosshare is explicitly rejected with the robots.txt reason; plain ASCII throughout.
...
### T67: Seed-only candidate recall eval across models
- Workstream: I (eval)
- Model: opus
- Depends on: T34, T47, T61
- Owns: scripts/eval-recall.ts, src/eval/recall.ts, test/unit/eval/recall.test.ts, test/fixtures/recall/** (hand-written fixtures); pre-authorised: docs/plan.md (this block, its index row and the Counts line), docs/benches/README.md (one new subsection describing the recall screen and its decision rule), docs/spec.md (one sentence in Metrics and run records pointing at the recall eval).
- Reads (must not edit): src/candidates/service.ts (getCandidates, getCandidatesBatch, purpose seed), src/llm/pricing.ts (capabilitiesOf, limitsOf, usdFor), src/llm/tierRouter.ts, src/profiles/builtins.ts and schema, src/puzzle/library.ts, src/eval/aggregate.ts, src/cli/bench.ts (how it wires transport, inference log, cache and the puzzle set), docs/benches/escalation-policy.md (the decomposition sections, for the metric definitions)
- Spec sections: Metrics and run records; Candidate service
- Deliverable: A model screen that isolates candidate generation from the solver. `npx tsx scripts/eval-recall.ts --set sets/modern-12.json --models <comma list> [--repeat 1] [--max-usd 10] [--out logs/recall]` runs ONLY the seed pass for every slot of every puzzle in the set, once per model (each model as tier 1 of a synthetic profile derived from baseline with tier1 = model; the router applies that model's capabilities: reasoning-off when advertised, structured outputs when supported), through the real CandidateService with the real cache (keys include the model, so re-runs are free) and inference log, and never assigns anything to a grid. `package.json` is frozen, so the screen has no npm alias: it runs as `npx tsx scripts/eval-recall.ts` inside the container. Per (model, puzzle, repeat) it records to `<out>/<model-slug>/<puzzle>--r<repeat>.json`: per slot truthInCandidates, truthRank, candidatesSeen (accepted), raw candidates offered, rejectCounts by reason, parse failure, clue_understood, latencyMs, tokens and usd (counterfactual and billed). It prints a table per model with columns: slots, truth-in-candidates share, top-1 share, mean candidatesSeen, length-error share of rejections, parse-failure rate, zero-candidate share, mean latency, usd per puzzle (counterfactual), and the same split by stratum; and writes `<out>/summary.md` plus `summary.json`. A pre-flight estimate (slots x models x tokens-per-call assumption x prices) is printed and the run refuses to start above `--max-usd` without `--yes`, mirroring bench. Rate limits: the process-wide limiter applies per model; run models sequentially, puzzles with the same concurrency as bench (2). The pure aggregation (src/eval/recall.ts: per-slot record type, aggregate by model and stratum, table and markdown rendering, estimate formula) is unit-tested against hand-written fixtures; the script is a thin runner tested only for argument parsing and the estimate/refusal path with a stub.
- Decisions baked in: seed pass only, no search, no re-asks, no escalation; the model is the tier-1 slot of a per-model profile so cache keys and inference-log purpose are exactly what a real solve's seed pass would use; models are run one after another; results go under logs/ (gitignored) and only aggregate numbers are ever committed; the screen's decision rule (recorded in docs/benches/README.md): rank models by truth-in-candidates share on the american stratum, then usd per puzzle; carry the top three plus the current tier-1 into the puzzle-level bench.
...
### T71: max-accuracy profile: lifted caps, reasoning on constrained calls, multi-sample voting on constrained calls
- Workstream: E
- Model: opus
- Depends on: T62, T65, T69
- Owns: src/profiles/builtins.ts, src/llm/tierRouter.ts, src/candidates/service.ts, test/unit/profiles/builtins.test.ts, test/unit/profiles/schema.test.ts, test/unit/llm/tierRouter.test.ts, test/unit/candidates/service.test.ts; pre-authorised: docs/plan.md (this block, its index row and the Counts line), src/profiles/schema.ts (the new `reasoning` group and `constrainedSamples` field only), src/events/types.ts only if a vote-merge event turns out to be needed (optional field additions preferred), docs/spec.md (Strategy profiles: the new fields and the two new built-ins; Candidate service: one paragraph on constrained-call sampling and reasoning).
- Reads (must not edit): src/llm/parser.ts (already strips think blocks and reasoning_content), src/llm/pricing.ts (capabilitiesOf), src/score/calibrate.ts, src/profiles/loader.ts, src/util/hash.ts, docs/crossword-algorithms.md (Calibration: sampling agreement; the CrossWordBench note that reasoning models gain from crossing letters), docs/benches/model-comparison.md
- Spec sections: Strategy profiles; Candidate service
- Deliverable: Three additions, each behind a profile field whose default keeps every existing profile's behaviour byte-identical. (1) Reasoning on constrained calls: new profile group `reasoning: { constrainedEffort: 'none' | 'low' | 'medium' | 'high' (default 'none'), constrainedMaxTokens: number (default 2048) }`. In tierRouter, for purposes that render the constrained or escalate templates (reask, repair, escalate) on a model that advertises reasoning, send `reasoning_effort` = constrainedEffort when it is not 'none' (respecting the per-model override table: a Harmony model gets the requested value directly, as it accepts low/medium/high) and set max_tokens to constrainedMaxTokens for those calls; seed calls keep the reasoning-off behaviour unchanged, and so does every call on a model that does not advertise reasoning. The cache key must include the effective reasoning effort and max tokens for the call (they change the response); max tokens is already a B23 key field, and the effort is folded in by the service next to them, with a contract-style unit test showing that a constrained request under 'none' and under 'medium' yields different keys while seed requests are unaffected. (2) Multi-sample voting on constrained calls: new profile field `constrainedSamples: number` (int, 1 to 5, default 1). When above 1, the service issues that many constrained-template requests for the slot with sampleIndex 0..N-1 (offset by the run's repeat index so two repeats never share a key; with the default repeat index 0 the indices are exactly 0..N-1), waits for all, and merges the accepted candidates by normalised answer: score = the best single-sample score plus 0.15 per additional sample that proposed the same answer (cap 1.0), with candidates ordered by votes desc then score desc then answer. It emits the merged list as one `slot:candidates`; the inference log records each sample; the events carry one `slot:ask` per sample (no new event type). Seed calls are never sampled here (votes3 calibration remains T53's). (3) Built-in `max-accuracy`: strong-only's models (tier1 and tier2 deepseek-ai/DeepSeek-V4-Pro) with escalation.maxTier2CallsPerPuzzle 200, escalationsPerSlot 2, reasksPerSlot 3, search.maxBacktracks 1000, repair.maxCalls 200, repair.maxEditDistance 2, budget.usd 2.0, budget.tokens 10000000, reasoning.constrainedEffort 'medium', constrainedSamples 3, promptVersion 2, everything else as baseline. Also add `strong-only-uncapped` (strong-only with the same lifted caps and budget but reasoning 'none' and constrainedSamples 1) so the levers can be separated in a bench. Update docs/spec.md.
- Decisions baked in: defaults preserve behaviour (proved by the offline integration tests passing with no snapshot change); the two new fields are **optional** in `ProfileSchema` rather than `.default()`-ed, because `schemas/run-record.schema.json` closes the profile object with `additionalProperties: false` and `test/contract/schemas.test.ts` types a complete `Profile` literal - both frozen - so a field every profile carries would fail the contract suite; an absent field means the documented default, applied by the two accessors `reasoningOf()` and `constrainedSamplesOf()` exported next to the fields, and every pre-existing built-in stays byte-identical. The reasoning value for constrained calls flows through the same override table as T68. Per-sample cache keys via sampleIndex. The vote merge is deterministic (stable sort by votes, then score, then answer) and the samples are issued sequentially, so the event and inference-log order of a sampled ask does not depend on which call returned first.
